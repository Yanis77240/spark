...
comparison_file=$2 # Second argument passed to the script
...
if [ -s "$file" ]; then # if the file is not empty
    ...
    curl -v -u $user:$pass --upload-file "$file" http://10.10.10.11:30000/repository/scala-test-comparison/spark/comparison-"$build_number"-"$comparison_number"
