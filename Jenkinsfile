        ...
        container('tdp-builder') {
            ...
            stage('Test') {
                ...
                withEnv(["number=${currentBuild.number}"]) {
                    ...
                    withCredentials([usernamePassword(credentialsId: '4b87bd68-ad4c-11ed-afa1-0242ac120002', passwordVariable: 'pass', usernameVariable: 'user')]) {
                        sh '''
                        grep -F --color=never --no-group-separator "*** FAILED ***" */target/surefire-reports/SparkTestSuite.txt */**/target/surefire-reports/SparkTestSuite.txt > scala-tests.txt
                        ...
                        '''
                        sh'''
                        mysql -h 10.100.99.143 -u root -padmin tests -e "CREATE TABLE spark_2_test_list (Module VARCHAR(125), Message VARCHAR(255)); LOAD DATA LOCAL INFILE 'scala-tests.txt' INTO TABLE spark_2_test_list FIELDS TERMINATED BY 'txt:' LINES TERMINATED by '\n'; UPDATE spark_2_test_list SET Module = REPLACE(Module, '/target/surefire-reports/SparkTestSuite.', '');"
                        '''
                        sh '''
                        mysql -h 10.100.99.143 -u root -padmin tests -e "CREATE TABLE spark_2_aborted_tests (Module VARCHAR(125), Results VARCHAR(255)); LOAD DATA LOCAL INFILE 'aborted-tests.txt' INTO TABLE spark_2_aborted_tests FIELDS TERMINATED BY 'txt:' LINES TERMINATED by '\n'; UPDATE spark_2_aborted_tests SET Module = REPLACE(Module, '/target/surefire-reports/SparkTestSuite.', '');"
                        '''
                        sh '''
                        mysql -h 10.100.99.143 -u root -padmin tests -e "CREATE TABLE spark_2_test_resume (Module VARCHAR(125), Results VARCHAR(255)); LOAD DATA LOCAL INFILE 'scala-end-results.txt' INTO TABLE spark_2_test_resume FIELDS TERMINATED BY 'txt:' LINES TERMINATED by '\n'; UPDATE spark_2_test_resume SET Module = REPLACE(Module, '/target/surefire-reports/SparkTestSuite.', ''); CREATE TABLE spark_2_results (SELECT Module, SUBSTRING_INDEX(Results, ',', 1) AS Succeeded,     SUBSTRING_INDEX(SUBSTRING_INDEX(Results, ',', 2), ',', -1) AS Failed,     SUBSTRING_INDEX(SUBSTRING_INDEX(Results, ',', 3), ',', -1) AS Canceled, SUBSTRING_INDEX(SUBSTRING_INDEX(Results, ',', 4), ',', -1) AS Ignored, SUBSTRING_INDEX(SUBSTRING_INDEX(Results, ',', 5), ',', -1) AS Pending  FROM spark_2_test_resume); drop table spark_2_test_resume; UPDATE spark_2_results SET Succeeded = SUBSTRING_INDEX(Succeeded, ' ', -1), Failed = SUBSTRING_INDEX(Failed, ' ', -1), Canceled = SUBSTRING_INDEX(Canceled, ' ', -1), Ignored = SUBSTRING_INDEX(Ignored, ' ', -1), Pending = SUBSTRING_INDEX(Pending, ' ', -1);"
                        '''
                    }
                }
            }
