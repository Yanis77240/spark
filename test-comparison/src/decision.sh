...
if [ -s "$file" ]; then # if the file is not empty
    ...
    curl -v -u $user:$pass --upload-file "$file" hhttp://10.10.10.11:30000/repository/component-test-comparison/spark-2.3/spark-2.3.5-comparison-"$build_number".csv # Set the path to the test comparison folder in the external repository.
